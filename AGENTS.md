...
# Charte graphique — identité "Lucide"
...
- **Nom de l'app** : importer `APP_NAME` depuis `src/lib/constants.ts`,
  jamais de chaîne `"Lucide"` codée en dur dans un nouveau composant.
...
- **Typographie** (`next/font/google` dans `src/app/layout.tsx`) :
  - `font-display` (Fraunces) : titres H1/H2 et wordmark, jamais le corps de
    texte.
  - `font-sans` (IBM Plex Sans) : corps de texte, valeur par défaut.
  - `font-mono` (IBM Plex Mono) : tout chiffre clé — score, prix, rendement,
    cash-flow (voir `ScoreGauge`, `ScoreBadge`, `ApartmentsTable` pour
    l'exemple).
- **Logo** : `LucideMark` exporté depuis `src/components/Navbar.tsx` (motif
...
- **Fonds décoratifs** (hero, bannières) : dégradé subtil en `accent-50` /
  ...
  luxe", un filigrane du logo (`LucideMark`) en très faible opacité peut
